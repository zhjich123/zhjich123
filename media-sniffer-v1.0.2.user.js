// ==UserScript==
// @name         媒体嗅探器 Media Sniffer Pro v1.0.2
...
(function () {
    ...
    LANG.strings = {
        ...
        ,
            ...
            'cookie': 'Cookie:',
            'infoLine1': '媒体嗅探器 Pro v1.0.2 · SelectionManager · 拖拽排序 · 收藏夹 · 智能去重 · 分组 · 批量操作注册',
        ...
        ,
            ...
            'userAgent': 'User-Agent:',
            'cookie': 'Cookie:',
            'infoLine1': 'Media Sniffer Pro v1.0.2 · SelectionManager · Drag Sort · Favorites · Smart Dedup · Groups · Batch Actions',
    ...
    Selection._updateCardMark = function (url) {
        ...
        if (mark) {
            if (State.selectionMode) {
                ...
                mark.style.background = isSel ? '#6366f1' : 'rgba(255,255,255,.9)';
                mark.style.border = isSel ? 'none' : '2px solid #6366f1';
                mark.style.color = isSel ? '#fff' : '#6366f1';
                mark.textContent = isSel ? '\u2713' : '';
            } else {
                mark.style.display = 'none';
            }
        }
        card.style.border = isSel ? '2px solid #6366f1' : (State.selectionMode ? '1px dashed #6366f1' : '');
    ...
    Selection._updateAllCards = function () {
        ...
        for (var j = 0; j < cards.length; j++) {
            ...
            var isSel = State.selected.has(url);
            if (State.selectionMode) {
                cards[j].style.border = isSel ? '2px solid #6366f1' : '1px dashed #6366f1';
    ...
    UI.applyUiStyle = function () {
        ...
        if (style === 'material') {
            ...
            css += '#_ms_panel._ms_style_apple { background:' + bg + ' !important; backdrop-filter:blur(26px) saturate(180%) !important; -webkit-backdrop-filter:blur(26px) saturate(180%) !important; border-left:1px solid ' + border + ' !important; box-shadow:' + shadow + ' !important; border-radius:' + r2 + ' !important; }';
            css += '#_ms_panel._ms_style_apple > div:first-child { background:' + headerBg + ' !important; border-bottom:1px solid ' + border + ' !important; }';
            css += '#_ms_panel._ms_style_apple #_ms_tabs { background:' + tabBg + ' !important; border-bottom:1px solid ' + border + ' !important; }';
            css += '#_ms_panel._ms_style_apple #_ms_search, #_ms_panel._ms_style_apple #_ms_filter, #_ms_panel._ms_style_apple #_ms_progress { background:' + tabBg + ' !important; border-bottom:1px solid ' + border + ' !important; }';
            css += '#_ms_panel._ms_style_apple #_ms_box, #_ms_panel._ms_style_apple #_ms_footer { background:transparent !important; border-top:1px solid ' + border + ' !important; }';
            css += '#_ms_panel._ms_style_apple #_ms_footer { border-top:1px solid ' + border + ' !important; }';
            css += '#_ms_panel._ms_style_apple [data-url] { background:rgba(255,255,255,0.08) !important; border:1px solid ' + border + ' !important; border-radius:16px !important; }';
            // 苹果风格：右下角浮动按钮也使用玻璃拟态
            ...
            css += '#_ms_float._ms_btn_apple { background:' + btnBg + ' !important; backdrop-filter:blur(16px) saturate(160%) !important; -webkit-backdrop-filter:blur(16px) saturate(160%) !important; border:1px solid ' + btnBorder + ' !important; box-shadow:' + btnShadow + ' !important; }';
        } else {
    ...
    UI.buildPanel = function () {
        ...
        State.panel.style.cssText = 'position:fixed;right:0;top:0;bottom:0;width:' + w + 'px;max-height:100vh;max-width:92vw;background:' + c.bg + ';color:' + c.txt + ';border-radius:' + (isMob ? '0' : '20px 0 0 20px') + ';box-shadow:-30px 0 60px rgba(0,0,0,.35);display:none;flex-direction:column;overflow:hidden;z-index:2147483645;transform:translateX(100%);transition:transform .35s cubic-bezier(.22,1,.36,1), background-color 0.3s, color 0.3s;font-family:system-ui,-apple-system,"PingFang SC","Microsoft YaHei",sans-serif;';
        ...
        progressWrap.style.cssText = 'padding:8px 12px;background:' + c.bg2 + ';border-bottom:1px solid ' + c.border + ';display:none;transition: background-color 0.3s, color 0.3s;';
    ...
    UI._renderMediaCard = function (url, idx, kind) {
        ...
        var borderStyle = isSel ? '2px solid #6366f1' : (inSelMode ? '1px dashed #6366f1' : '1px solid ' + c.border);
        ...
        markHtml = '<div class="_ms_sel_mark" style="position:absolute;top:' + (isMobile ? '8px' : '6px') + ';right:' + (isMobile ? '8px' : '6px') + ';width:' + markSize + ';height:' + markSize + ';border-radius:50%;' + (isSel ? 'background:#6366f1;color:#fff;' : 'background:rgba(255,255,255,.9);border:2px solid #6366f1;color:#6366f1;') + 'font-size:' + markFontSize + ';font-weight:700;display:' + markDisplay + ';align-items:center;justify-content:center;z-index:2;">' + (isSel ? '✓' : '') + '</div>';
        var iframeBadge = '';
        if (kind === 'video' && Scanner._iframeVideoUrls && Scanner._iframeVideoUrls.has(url)) {
            iframeBadge = '<div style="position:absolute;top:' + (isMobile ? '8px' : '6px') + ';left:' + (isMobile ? '8px' : '6px') + ';padding:2px 6px;border-radius:4px;background:rgba(0,0,0,.6);color:#fff;font-size:10px;font-weight:600;z-index:2;pointer-events:none;">' + LANG.t('iframeBadge') + '</div>';
        }
        return '<div class="_ms_card" data-url="' + url + '" data-idx="' + idx + '" draggable="' + (inSelMode ? 'true' : 'false') + '" style="position:relative;border-radius:10px;background:' + c.bg2 + ';overflow:hidden;cursor:pointer;' + borderStyle + shadow + ';">' +
    ...
    UI.renderSettings = function () {
        ...
        for (var si = 0; si < styles.length; si++) {
            (function (s) {
                ...
                b.style.cssText = 'flex:1;min-width:80px;padding:10px;border:none;border-radius:10px;background:' + (active ? 'linear-gradient(135deg,' + c.primary + ',' + c.primary2 + ')' : c.bg3) + ';color:' + (active ? '#fff' : c.txt) + ';font-size:13px;font-weight:600;cursor:pointer;' + (active ? 'box-shadow:0 4px 12px ' + c.primary + '4d;' : '');
        ...
        palRow.style.cssText = 'display:grid;grid-template-columns:repeat(auto-fill, minmax(64px, 1fr));gap:10px;';
        ...
        container.appendChild(makeGroup('palette', LANG.t('grpPalette'), palRow));
